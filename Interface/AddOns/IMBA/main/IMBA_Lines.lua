...
function IMBA_DrawLine(C, sx, sy, ex, ey, w, color)
   ...
   for k,v in pairs(IMBA_Lines[C:GetName()]) do
...
function IMBA_EraseLines(C,x,y,r)
	...
	for k,v in pairs(IMBA_Line_Data[C:GetName()]) do
...
function IMBA_CreateSaveImage(C)
	...
	for k,v in pairs(IMBA_Line_Data[C:GetName()]) do
		...
		AddedLine=false;
		if IMBA_Lines[C:GetName()][k]:IsShown() then
			for k2, Stroke in pairs(ImageStrokes) do
...
function IMBA_DrawSavedImage(C,Image)
	...
	for k, Stroke in pairs(Image) do
		local lines;
...
function IMBA_LineMsgHandler(canvas)	
	if arg1=="IMBA_LINES_"..canvas:GetName() and arg4~=UnitName("player") then
		...
		for sx, sy, ex, ey, r, g, b, a in string.gmatch(arg2, "DRAWLINE (%d+.?%d*) (%d+.?%d*) (%d+.?%d*) (%d+.?%d*) (%d+.?%d*) (%d+.?%d*) (%d+.?%d*) (%d+.?%d*)") do
			IMBA_DrawLine(canvas,sx,sy,ex,ey,32,{r,g,b,a})
		end

		for x,y in string.gmatch(arg2, "ERASE (%d+.?%d*) (%d+.?%d*)") do
		...
		if string.find(arg2,"DRAWSTROKE") then
			...
			local x, y
			for x, y in string.gmatch(rest,"a (%d+.?%d*) (%d+.?%d*)") do
