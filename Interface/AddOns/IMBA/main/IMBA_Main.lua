...
IMBA_LOCATIONS_KAZA = "Karazhan";

IMBA_LOCATIONS_OTHER = "Other";


IMBA_Locations = {{IMBA_LOCATIONS_KAZA,0}, {IMBA_LOCATIONS_MC,0}, {IMBA_LOCATIONS_BWL,0}, {IMBA_LOCATIONS_ZG,0}, {IMBA_LOCATIONS_AQ20,0}, {IMBA_LOCATIONS_AQ40,0}, { IMBA_LOCATIONS_NAXX_ABOM, 0 }, { IMBA_LOCATIONS_NAXX_DK, 0 }, { IMBA_LOCATIONS_NAXX_PLAGUE, 0 }, { IMBA_LOCATIONS_NAXX_SPIDER, 0 }, { IMBA_LOCATIONS_NAXX_LAIR, 0}, { IMBA_LOCATIONS_OTHER, 0}};
...
function IMBA_SetBackdrops(num)
	IMBA_BossFrame:SetBackdrop(IMBA_BG[num].table);	
	IMBA_BossFrame:SetBackdropBorderColor(IMBA_SavedVariables.Colors["FrameBorder"].r,IMBA_SavedVariables.Colors["FrameBorder"].g,IMBA_SavedVariables.Colors["FrameBorder"].b,IMBA_SavedVariables.Colors["FrameBorder"].a);	
	IMBA_BossFrame:SetBackdropColor(IMBA_SavedVariables.Colors["FrameBG"].r,IMBA_SavedVariables.Colors["FrameBG"].g,IMBA_SavedVariables.Colors["FrameBG"].b,IMBA_SavedVariables.Colors["FrameBG"].a);	

	IMBA_BossFrame_Close_Image:SetTexture(IMBA_BG[num].close);
	IMBA_BossFrame_Close_Image:SetVertexColor(IMBA_SavedVariables.Colors["FrameBorder"].r,IMBA_SavedVariables.Colors["FrameBorder"].g,IMBA_SavedVariables.Colors["FrameBorder"].b,IMBA_SavedVariables.Colors["FrameBorder"].a);

	for k, v in pairs(IMBA_Addons) do
...
function IMBA_ResetPositions()
	...
	IMBA_BossFrame:ClearAllPoints();
	IMBA_BossFrame:SetPoint("CENTER", "UIParent", "CENTER", 0, 0);
end
...
function IMBA_CreateFrameFunc(AddonName,Func)
	IMBA_Addons[AddonName].BossFrameFunc=Func
end
...
function IMBA_CheckSyncVariables(Addon)
	for k, v in pairs(Addon.SyncVariablesLastVal) do
...
function IMBA_OnUpdate()
	...
	if IMBA_CheckRegen and (IMBA_CheckRegenTimer < GetTime()) then
		...
		--Safety check in case they bloodraged
		if IMBA_NumRaidInCombat()/(GetNumRaidMembers()+1)>0.5 then
			for k, v in pairs(IMBA_Addons) do
...
function IMBA_OnEvent(event)
	...
	elseif event=="CHAT_MSG_ADDON" then
		if arg1=="IMBA" then
			...
			if commandName=="VARSYNC" then
				...
				for k, v in pairs(IMBA_Addons) do
					for k2, v2 in pairs(v.SyncVariablesLastVal) do		
						if k2==VarName then
	...
	elseif event=="PLAYER_LOGIN" then
		if IMBA_SavedVariables then
			for k2, v2 in pairs(IMBA_Addons) do
				if v2.MainFrame then
					local FoundModSettings;
					FoundModSettings=false
					for k, v in pairs(IMBA_SavedVariables.Mods) do		
						if k==k2 then
							v2.Active=v.Active;
							if v2.MainFrame then
								if v.Visible then
	...
	elseif event=="CHAT_MSG_COMBAT_HOSTILE_DEATH" then
		...
		if MobName then
			if IMBA_BossFrame:IsShown() then
				for k,v in pairs(IMBA_BossFrame_CurrentBoss) do
					if v==MobName then
						tremove(IMBA_BossFrame_CurrentBoss,k)
					end
				end

				if getn(IMBA_BossFrame_CurrentBoss)==0 then
					IMBA_BossFrame:Hide()
				end
			end
			for k, v in pairs(IMBA_Addons) do
				if v.MainFrame and getglobal(v.MainFrame):IsShown() then						
...
function IMBA_SetScale(loading)
		if IMBA_BossFrame then
			if not loading then 
				local pointNum=IMBA_BossFrame:GetNumPoints()
				local curScale=IMBA_BossFrame:GetScale();
				local pointsCleared=false
				local points={}
				for i=1,pointNum,1 do
					points[i]={};
					points[i][1], points[i][2], points[i][3], points[i][4], points[i][5]=IMBA_BossFrame:GetPoint(i)
				...
				for i=1,pointNum,1 do
					if points[i][1] and points[i][2] and points[i][3] and points[i][4] and points[i][5] then
						...
						IMBA_BossFrame:SetPoint(points[i][1],UIParent,points[i][3],points[i][4],points[i][5]);
					end
				end
			end
		
			IMBA_BossFrame:SetScale(IMBA_SavedVariables.Scale);
		end
