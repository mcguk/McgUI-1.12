...
function IMBA_SortTable(t1, t2)
	local locs = { };
	for k, v in pairs(IMBA_Locations) do
		locs[v[1]] = k;
	end
...
function IMBA_InitDropDown()
	...
	if IMBA_Addons[modName].BossFrameFunc then
		info = { };
		info.text = "Show";
		info.tooltipTitle = "Show";
		info.tooltipText = "Shows this boss mod in the generic boss frame (If the addon is enabled it will auto become visible at the beginning of the boss fight)";
		info.func = IMBA_ShowBossAddon;
		info.value = modName;
		UIDropDownMenu_AddButton(info);	
	end
	...
	if IMBA_Addons[modName].Options then
		for k, v in pairs(IMBA_Addons[modName].OptionsOrder) do
	...
	if IMBA_Addons[modName].VarText then
		for k, option in pairs(IMBA_Addons[modName].OptionsOrder2) do
			info = { };
...
function IMBA_ShowBossAddon()
	if IMBA_Addons[this.value].BossFrameFunc then
		IMBA_Addons[this.value]:BossFrameFunc()
		IMBA_BossFrame:Show()
	end
end
