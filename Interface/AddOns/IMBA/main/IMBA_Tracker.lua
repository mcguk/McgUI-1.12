...
function IMBA_Tracker_OnEvent(event)
	...
	elseif event=="CHAT_MSG_ADDON" then
		if arg1=="IMBA_TRACKER_UPDATE" then
			...
			elseif string.find(arg2,"CALIBRATION") and IMBA_Tracker_ValidatePos then
				...
				for k,v in pairs(IMBA_Tracker_Calibration) do
	...
	elseif event=="PLAYER_LOGIN" then
		...
		if IMBA_SavedVariables.LastPlayerPos then
			...
			IMBA_SavedVariables.LastPlayerPos={x=0;y=0}
		end
	end
end
...
function IMBA_Tracker_OnUpdate()
	...
	if IMBA_Tracker_Loaded then
		IMBA_SavedVariables.LastPlayerPos.x=x
		IMBA_SavedVariables.LastPlayerPos.y=y
	end
	...
	if IMBA_Tracker_Calibrating and IMBA_Tracker_CountTime<GetTime() then
		...
		if IMBA_Tracker_Calibration and IMBA_Tracker_Calibration[1].score>1.5 then
	...
	if not IMBA_Tracker_ValidatePos and (IMBA_Tracker_EnabledRaid or IMBA_Tracker_EnabledGuild)  and IMBA_Tracker_UpdateTime<GetTime() then
		...
		SetMapToCurrentZone()
		local msg=string.format("%d %.3f %.3f %.3f %d %d %d %d %d %s %d %d",GetCurrentMapContinent(),x,y,ang,UnitHealth("player"),UnitHealthMax("player"),UnitMana("player"),UnitManaMax("player"),UnitPowerType("player"),englishClass,effectiveArmor,defense);
