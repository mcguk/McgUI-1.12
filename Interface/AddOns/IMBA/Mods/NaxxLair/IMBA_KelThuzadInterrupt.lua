...
function IMBA_KelThuzadInterrupt_PlayerKicks(name)
	for k,v in pairs(IMBA_KelThuzadInterrupt_Players) do
		if v[1]==name then
			v[2]=GetTime()+10;
			v[3]=10;
			v[4]="Kick";
...
function IMBA_KelThuzadInterrupt_OnUpdate()
	...
	for k,v in pairs(IMBA_KelThuzadInterrupt_Players) do
		if UnitIsDeadOrGhost(v[5]) then
			tremove(IMBA_KelThuzadInterrupt_Players,k)
		end
	...
	for i=1,15,1 do
		if i<=numEntries then
			...
			if (IMBA_KelThuzadInterrupt_Players == nil or IMBA_KelThuzadInterrupt_Players[i] == nil) then
			break
			else
	...
	if numEntries>15 then
		numEntries=15;
	end
	IMBA_KelThuzadInterrupt:SetHeight(12*20+70);
end
...
function IMBA_KelThuzadInterrupt_OnLoad()
	...
	for i=1,15,1 do
		getglobal("IMBA_KelThuzadInterrupt_Icon"..i):SetIcon("Interface\\AddOns\\IMBA\\textures\\WhiteArrow.blp");
	end
...
function IMBA_KelThuzadInterrupt_OnEvent(event)
	if event=="CHAT_MSG_SPELL_CREATURE_VS_CREATURE_DAMAGE" then
		if arg1==IMBA_KelThuzad_Frostbolt_Msg then
	...
	elseif (event=="CHAT_MSG_SPELL_SELF_DAMAGE") or (event=="CHAT_MSG_SPELL_PARTY_DAMAGE") or (event=="CHAT_MSG_SPELL_FRIENDLYPLAYER_DAMAGE") or (event=="CHAT_MSG_SPELL_HOSTILEPLAYER_DAMAGE") then
		...
		if IMBA_CheckVar("Kel'Thuzad - Interrupt Assister","TrackRogues")==false then
			iStart, iEnd, sPlayer = string.find(arg1, IMBA_Interrupt_Kick);
			if ( sPlayer ) then
		...
		if IMBA_CheckVar("Kel'Thuzad - Interrupt Assister","TrackWarriors")==false then
			iStart, iEnd, sPlayer = string.find(arg1, IMBA_Interrupt_Pummel);
			if ( sPlayer ) then
				if ( sPlayer == "Your") then
			...
			iStart, iEnd, sPlayer = string.find(arg1, IMBA_Interrupt_ShieldBash);
			if ( sPlayer ) then
				if ( sPlayer == "Your") then
		...
		if string.find(arg1,IMBA_KelThuzad_Frostbolt_Interrupted_Msg) then
			IMBA_KelThuzadInterrupt_TimerFrostbolt.timeEnd=0;
