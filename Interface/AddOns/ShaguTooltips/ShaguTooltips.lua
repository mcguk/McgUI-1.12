...
ShaguUITooltipStatusBar:SetScript("OnUpdate", function()
   ...
   -- if hp and hpm then
	...
	local p=format("%.2f",(UnitHealth("mouseover") / UnitHealthMax("mouseover")) * 100)
	ShaguUITooltipStatusBar.HP:SetText(p .."%")

    if class and reaction then
      if UnitIsPlayer("mouseover") then
        local color = RAID_CLASS_COLORS[class]
        --GameTooltipStatusBar:SetStatusBarColor(color.r, color.g, color.b)
		GameTooltipStatusBar:SetStatusBarColor(0.149, 0.149, 0.149)
      else
...
ShaguUITooltipEvent:SetScript("OnEvent", function()
    ...
    --if guild then
      --GameTooltip:AddLine("<" .. guild .. ">", 0, 1, 0)
   -- end
        ...
        GameTooltip:AddLine("<" .. guild .. ">" .. lead .. rank, 0, 1, 0)
      end
